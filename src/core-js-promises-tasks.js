...
function getFirstResolvedPromiseResult(promises) {
  const checkPromise = (i) =>
    i < promises.length
      ? promises[i]
          .then((v) => Promise.resolve(v))
          .catch(() => checkPromise(i + 1))
      : Promise.reject();
  return checkPromise(0);
}
...
function getAllResult(promises) {
  return Promise.allSettled(promises).then((vs) =>
    vs.map((v) => (v.status === 'fulfilled' ? v.value : null))
  );
}
...
function queuPromises(promises) {
  const processPromise = (acc, i) =>
    i < promises.length
      ? promises[i]
          .then((v) => processPromise(`${acc}${v}`, i + 1))
          .catch(() => processPromise(acc, i + 1))
      : Promise.resolve(acc);
